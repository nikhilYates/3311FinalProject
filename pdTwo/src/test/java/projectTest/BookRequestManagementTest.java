...
public class BookRequestManagementTest {
	...
	public void testSpecifyRequestDetailsFound() {
		...
		BookRequest request = manager.getRequests().stream().filter(r -> r.getRequestID().equals(requestID)).findFirst()
				.orElse(null);
	...
	public void testPrioritizeRequestFound() {
		...
		// Assert that the request was prioritized, assuming there's a getter for
		// requests (for test purposes)
		BookRequest request = manager.getRequests().stream().filter(r -> r.getRequestID().equals(requestID)).findFirst()
				.orElse(null);
		assertNotNull(request);
	...
	public void testGetRequestsSystemManagerOnlyAuthorized() {
		// Access the singleton instance of SystemManager using the static getInstance
		// method
