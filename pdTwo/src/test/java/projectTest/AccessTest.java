...
class AccessTest { // xlsx really messy cause i only can read it
	...
	public void signupFailedTestStudent() throws IOException {
		boolean signingup = User.signup(this.loginFailUserId, this.loginFailEmail, this.loginFailPassword,
				Student.class);
		assertFalse(signingup);
	}
	...
	public void studentMethod2Test() throws IOException { // validated student using temp method
		...
		String major = "Computer Science";
		student.setMajor(major);
		student = User.login(this.studentEmail, this.studentPassword); // basically logging in again to check i major is
																		// in cvs file, and is retrieved when making the
																		// object
		assertEquals(student.getMajor(), major);
	}
