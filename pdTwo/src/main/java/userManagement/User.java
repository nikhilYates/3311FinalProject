...
public abstract class User {
	...
	public void comprehensiveReport() {
		...
		if (lateItems.size() >= 3) {
			...
			System.out.println(
					"ALERT. Your account's ability to rent items has been suspended due to at least 3 late items. Please return these items to regain rent ability.");
		}
		...
		if (totalLateBalance > 0) {
			...
			System.out.println("ALERT. You have an outstanding balance of $" + totalLateBalance
			...
			if (email.equalsIgnoreCase(row.getCell(EMAILCELL).getStringCellValue())) { // based on current positioning
				if (password.equalsIgnoreCase(row.getCell(PASSWORDCELL).getStringCellValue())) {
					...
					if (userType.equalsIgnoreCase(Faculty.class.getSimpleName())) {
						return new Faculty((int) row.getCell(USERIDCELL).getNumericCellValue(), email, thisRowNum,
								row.getCell(DEPARTMENTCELL).getStringCellValue());
						...
						return new Student((int) row.getCell(USERIDCELL).getNumericCellValue(), email, thisRowNum,
								row.getCell(MAJORCELL).getStringCellValue(),
								(int) row.getCell(YEARCELL).getNumericCellValue());
	...
	public static boolean signup(int userId, String email, String password, Class<? extends User> userType)
			throws IOException {
	...
	// add a row delete method, wither static and find email and delete, or via
	// class and change constructor to initiate with row instead of email
	// or do both, one can be for testing and one for client, testing will be the
	// static
